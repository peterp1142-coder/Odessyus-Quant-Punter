...
router.get('/', async (_req: Request, res: Response) => {
  try {
    const rows = await query<Record<string, unknown>[]>(
      `SELECT id, session_id, event_date, sport, fixture, league, prediction_market,
...
router.get('/:id', async (req: Request, res: Response) => {
  ...
  try {
    const rows = await query<Record<string, unknown>[]>(
      `SELECT p.*, fv.odds_data, fv.injuries_json, fv.player_data,
...
router.patch('/:id/status', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const rows = await query<Record<string, unknown>[]>('SELECT * FROM predictions WHERE id = ?', [id]);
    if (!rows.length) return res.status(404).json({ error: 'Not found' });
    res.json({ prediction: rows[0] });
  } catch (err) {
...
router.patch('/:id/result', async (req: Request, res: Response) => {
  const { id } = req.params;
  const { closingOdds, actualResult, status } = req.body as {
    closingOdds?: number;   // decimal odds the market closed at for our selection
    actualResult?: string;  // 'home_win' | 'draw' | 'away_win' | 'btts_yes' etc.
  ...
  try {
    const rows = await query<Record<string, unknown>[]>(
      'SELECT recommended_odds, probability, status FROM predictions WHERE id = ?', [id]
    );
    if (!rows.length) return res.status(404).json({ error: 'Not found' });
    ...
    let clvAchieved: number | null = null;
    ...
    if (resolvedStatus === 'won'  && openingOdds > 1) roi = openingOdds - 1;
    if (resolvedStatus === 'lost')                    roi = -1;
    if (resolvedStatus === 'void')                    roi = 0;
    ...
    const updated = await query<Record<string, unknown>[]>('SELECT * FROM predictions WHERE id = ?', [id]);
    res.json({
...
router.get('/stats/calibration', async (_req: Request, res: Response) => {
  try {
    const rows = await query<Record<string, unknown>[]>(`
      SELECT
        CASE
    ...
    const totalBets    = (rows as Record<string, unknown>[]).reduce((s, r) => s + Number(r.total_bets || 0), 0);
    const totalWins    = (rows as Record<string, unknown>[]).reduce((s, r) => s + Number(r.wins || 0), 0);
    const miscalibrated = (rows as Record<string, unknown>[]).some(r => r.calibration_status === 'MIS-CALIBRATED');
...
router.get('/stats/edge', async (_req: Request, res: Response) => {
  try {
    const rows = await query<Record<string, unknown>[]>(`
      SELECT
        id,
    ...
    const resolved = (rows as Record<string, unknown>[]).filter(r => r.clv_achieved !== null);
...
router.get('/stats/clv', async (_req: Request, res: Response) => {
  try {
    const rows = await query<Record<string, unknown>[]>(`
      SELECT
        COUNT(*)                                          AS total,
