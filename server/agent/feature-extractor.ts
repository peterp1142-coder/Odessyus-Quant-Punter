...
For odds, extract verification metadata only when explicitly supported by the OddsScout report: oddsVerified, independentSourceCount, oddsAgreementPct, oddsTimestamp, bestAvailableOdds, medianMarketOdds.
Return JSON with these exact fields: xgHome,xgAway,xgConcededHome,xgConcededAway,goalsScoredLast5Home,goalsScoredLast5Away,goalsConcededLast5Home,goalsConcededLast5Away,formPtsHome,formPtsAway,h2hWinRateHome,homeWinPctVenue,awayWinPctVenue,streakHome,streakAway,restDaysHome,restDaysAway,impliedProbHome,impliedProbDraw,impliedProbAway,selectionOdds,selectionImpliedProb,openingOddsHome,currentOddsHome,pinnacleOdds,overround,reverseLM,lineMovement,oddsVerified,independentSourceCount,oddsAgreementPct,oddsTimestamp,bestAvailableOdds,medianMarketOdds,injuryIndexHome,injuryIndexAway,absentPlayerRatingHome,absentPlayerRatingAway,gtdRiskHome,gtdRiskAway,motivationHome,motivationAway,weatherImpact,refBias,crowdFactor,tacticalEdge,refereeRelevance,lineupQualityHome,lineupQualityAway,marketLiquidity,bookmakerDispersion,modelRiskScore,dataQualityScore.`;
 try{
  const r=await mistralPool.call(c=>c.chat.complete({model:'mistral-small-latest',messages:[{role:'system',content:'Parse only supplied evidence. JSON only. Preserve nulls. Do not turn missing data into zero. Never infer verification from a single source.'},{role:'user',content:prompt}] as any,temperature:0,maxTokens:1500}));
  ...
  const formKeys=['xgHome','xgAway','xgConcededHome','xgConcededAway','goalsScoredLast5Home','goalsScoredLast5Away','goalsConcededLast5Home','goalsConcededLast5Away','formPtsHome','formPtsAway','h2hWinRateHome','homeWinPctVenue','awayWinPctVenue','streakHome','streakAway','restDaysHome','restDaysAway'];
  const oddsKeys=['selectionOdds','selectionImpliedProb','currentOddsHome','openingOddsHome','pinnacleOdds','overround','lineMovement','oddsVerified','independentSourceCount','oddsAgreementPct','bestAvailableOdds','medianMarketOdds'];
