...
interface AgentTask {
  ...
  focus: string;
  required: boolean;
  tier: 1 | 2 | 3;   // execution tier (1 and 2 run in parallel within tier)
}
...
export interface OrchestratorResult {
  ...
  metadata: {
    ...
    agentPlan: AgentTask[];
    agentsRun: string[];
    subagentResults: Record<string, boolean>;
    poolStatus: { total: number; available: number; cooled: number };
  };
}
...
): Promise<{ fixtures: string[]; rawSchedule: string }> {
...
Use exact team names. Skip matches from wrong dates.`,
    ...
    if (m) {
      const parsed = JSON.parse(m[0]) as { fixtures?: string[] };
...
async function parseQuery(userQuery: string): Promise<{ fixture: string; sport: string; market: string; matchDate: string }> {
  try {
    const resp = await mistralPool.call(client =>
      client.chat.complete({
        model: 'mistral-small-latest',
...
Return exactly: {"fixture":"Team A vs Team B or 'multiple/open'","sport":"football","market":"Match Result","matchDate":"today or YYYY-MM-DD"}`,
    ...
    const m    = text.match(/\{[\s\S]+\}/);
    if (m) {
      const p = JSON.parse(m[0]) as { fixture?: string; sport?: string; market?: string; matchDate?: string };
      return {
...
Return JSON only:
{
  ...
  try {
    ...
    const m = text.match(/\{[\s\S]+\}/);
    if (m) {
      const plan = JSON.parse(m[0]) as AgentPlan;
...
function buildPriorContext(accumulated: Record<string, SubAgentResult>): string {
  const entries = Object.entries(accumulated);
  if (entries.length === 0) return '';

  const lines: string[] = ['=== PRIOR CONTEXT FROM COMPLETED AGENTS ==='];
...
export async function runOrchestrator(
  ...
  const collectSteps = (step: ReActStep) => {
  ...
  const accumulated: Record<string, SubAgentResult> = {};
  const agentsRun: string[] = [];

  // Group agents by tier
  const byTier: Record<number, AgentTask[]> = {};
  for (const agent of plan.agents) {
    if (!byTier[agent.tier]) byTier[agent.tier] = [];
  ...
  for (const tier of tiers) {
    ...
    const tierResults = await Promise.allSettled(
      tierAgents.map(task => dispatchAgent(task, fixture, sport, matchDate, `${sessionId}-${task.name}`, fullContext, collectSteps, discoveredFixtures.length > 1 ? discoveredFixtures : undefined))
    );
  ...
  let quantResult: Awaited<ReturnType<typeof runQuantSynthesis>>;
  try {
