...
export async function allSportsFixtures(
  dateFrom: string,
  dateTo?: string,
): Promise<ToolResult> {
  ...
  for (let attempt = 0; attempt < pool.length; attempt++) {
    ...
    try {
      ...
      const res = await fetchWithTimeout(
        `${ALLSPORTS_URL}?${params}`,
        { headers: { 'Accept': 'application/json' } },
        SEARCH_TIMEOUT,
      );

      if (res.status === 401 || res.status === 403) { _allSportsIdx++; continue; }
      if (!res.ok) { _allSportsIdx++; continue; }
      ...
      const lines = json.result.map(e =>
        `${e.event_date} ${e.event_time || ''} | ${e.event_home_team} vs ${e.event_away_team}` +
...
export async function allSportsLivescore(): Promise<ToolResult> {
  ...
  for (let attempt = 0; attempt < pool.length; attempt++) {
    ...
    try {
      ...
      const lines = json.result.map(e =>
        `${e.event_home_team} ${e.event_home_team_score ?? '-'}:${e.event_away_team_score ?? '-'} ${e.event_away_team}` +
  ...
  for (let attempt = 0; attempt < pool.length; attempt++) {
    ...
    try {
      ...
      const envelope = await res.json() as {
        code: number;
        data?: { json?: Record<string, string>; html?: string };
        msg?: string;
      };
      ...
      if (organic.length === 0) { _taloredataIdx++; continue; }
...
export async function fetchUrl(url: string, useMobile = false): Promise<ToolResult> {
  try {
    const res = await fetchWithTimeout(
      url,
      { headers: { ...BROWSER_HEADERS, 'User-Agent': useMobile ? MOBILE_UA : DESKTOP_UA } },
      FETCH_TIMEOUT,
    );
    if (!res.ok) {
 ...
 */
async function discoverSelector(
  page: Page,
  hint?: string,
): Promise<string | null> {
  const candidates = await page.evaluate((h?: string) => {
    ...
    for (const el of elements) {
      ...
      if (h && text.toLowerCase().includes(h.toLowerCase())) score += 5;
...
export async function scrape(url: string, selector: string, waitTime = 7000): Promise<ToolResult> {
  ...
  if (wsUrl || !scraperToken) {
    ...
    try {
      ...
      let text = await page.evaluate((sel: string) => {
  ...
  try {
    ...
    const json = await res.json() as { status?: string; data?: string; error?: string };
...
export async function fetchMatchesToday(sport = 'football', dateStr?: string): Promise<ToolResult> {
  ...
  const puppeteerResults = await Promise.allSettled(
    puppeteerSources.map(({ url, selector, waitTime }) => scrape(url, selector, waitTime)),
  );
...
export async function fetchFbrefStats(team: string, league?: string): Promise<ToolResult> {
  ...
  const directResult = await scrape(
    `https://fbref.com/en/squads/${teamSlug}/`,
    '#stats_shooting, #stats_standard, body',
    8000,
  );
...
// ─── Tool: book_slip ───────────────────────────────────────────────────────
export async function bookSlip(url: string, selector: string, waitTime = 7000): Promise<ToolResult> {
...
export async function placeBetTool(input: Record<string, unknown>): Promise<ToolResult> {
  try {
    ...
    const result = await placeBet({
      platform:    String(input.platform || process.env.BOOKING_PLATFORM || 'sportybet') as import('../booking/types.js').PlatformId,
...
export async function dispatchTool(toolName: string, toolInput: Record<string, unknown>): Promise<ToolResult> {
  ...
  switch (toolName) {
    ...
    case 'allsports_fixtures':
      return allSportsFixtures(
        String(toolInput.date || toolInput.from || new Date().toISOString().slice(0, 10)),
        toolInput.to ? String(toolInput.to) : undefined,
      );
    ...
    case 'book_slip':
      return bookSlip(
        String(toolInput.url || toolInput.bookmaker_url || ''),
        String(toolInput.selector || 'body'),
        Number(toolInput.waitTime ?? 7000),
      );

    case 'fetch_matches_today':
      return fetchMatchesToday(
        String(toolInput.sport || 'football'),
    ...
    case 'fetch_fbref_stats':
      return fetchFbrefStats(
        String(toolInput.team || toolInput.fixture || ''),
        String(toolInput.league || ''),
      );
    ...
    case 'calculate_kelly':
      return calculateKelly(
        Number(toolInput.true_probability || toolInput.prob || 0),
        Number(toolInput.decimal_odds || toolInput.odds || 0),
        Number(toolInput.bankroll || 1000),
      );
