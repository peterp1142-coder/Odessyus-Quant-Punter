...
async function getBrowser():Promise<Browser>{
  ...
  const candidates=[process.env.PUPPETEER_EXECUTABLE_PATH,'/usr/bin/chromium','/usr/bin/chromium-browser','/nix/var/nix/profiles/default/bin/chromium','/nix/var/nix/profiles/default/bin/chromium-browser'];
  let executablePath=''; for(const p of candidates.filter(Boolean) as string[]){try{if(fs.existsSync(p)){executablePath=p;break;}}catch{}}
  if(!executablePath) executablePath='chromium';
  browser=await puppeteer.launch({headless:true,executablePath,args:['--no-sandbox','--disable-setuid-sandbox','--disable-dev-shm-usage','--disable-gpu','--disable-extensions','--disable-component-update','--disable-background-networking','--no-first-run','--no-default-browser-check','--password-store=basic','--use-mock-keychain']});
  return browser;
}

async function navigate(page:Page,url:string,waitTime:number){await page.setUserAgent(DESKTOP_UA);await page.setExtraHTTPHeaders({'Accept-Language':'en-US,en;q=0.9',Accept:'text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8'});await page.goto(url,{waitUntil:'domcontentloaded',timeout:30000});await new Promise(r=>setTimeout(r,waitTime));}

async function discoverSelectors(page:Page,hint:string):Promise<string[]>{return await page.evaluate((h)=>{const els=Array.from(document.querySelectorAll('div,table,ul,ol,section,article,main,tbody,tr'));const out:{s:string;n:number}[]=[];for(const el of els){const text=(el as HTMLElement).innerText||'';if(text.length<100||text.length>30000)continue;let n=0;if(/\b[1-9]\.\d{2}\b/.test(text))n+=3;if(/[A-Z][a-z]+\s+[-–]\s+[A-Z][a-z]+/.test(text))n+=2;if(/\b\d{1,2}:\d{2}\b/.test(text))n+=2;if(/odds|bet|kickoff|vs|match|fixture|score/i.test(text))n++;if(h&&text.toLowerCase().includes(h.toLowerCase()))n+=2;if(n<3)continue;const id=(el as HTMLElement).id, cls=(el as HTMLElement).className;let s=el.tagName.toLowerCase();if(id)s=`#${id}`;else if(typeof cls==='string'&&cls.trim())s=`${s}.${cls.trim().split(/\s+/)[0].replace(/[^a-zA-Z0-9_-]/g,'')}`;out.push({s,n});}return [...new Set(out.sort((a,b)=>b.n-a.n).slice(0,12).map(x=>x.s))];},hint);}
...
export async function scrape(url:string,selector:string,waitTime=7000):Promise<ToolResult>{let page:Page|null=null;try{const b=await getBrowser();page=await b.newPage();await navigate(page,url,waitTime);const key=cacheKey(url,selector);const candidates=[selectorCache.get(key),selector].filter(Boolean) as string[];let text='';for(const s of candidates){try{const t=await extract(page,s);if(t.length>=100){text=t;break;}}catch{}}
  if(text.length<100){console.log(`[SCRAPE] Selector mismatch: ${selector} @ ${url}; inspecting live DOM`);for(const s of await discoverSelectors(page,selector)){try{const t=await extract(page,s);if(t.length>=100){text=t;selectorCache.set(key,s);console.log(`[SCRAPE] Recovered selector: ${s}`);break;}}catch{}}}
  ...
  if(text.length<100)return{success:false,data:'',error:'Scrape yielded insufficient content after local selector recovery',source:'scrape:local-browser'};if(isBlocked(text))return{success:false,data:text,blocked:true,error:'Content blocked',source:'scrape:local-browser'};return{success:true,data:truncate(text,14000),source:'scrape:local-browser'};
}catch(e){return{success:false,data:'',error:String(e),source:'scrape:local-browser'};}finally{if(page)await page.close().catch(()=>{});}}
