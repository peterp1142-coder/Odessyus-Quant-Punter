...
function classify(text: string) {
  ...
  const tacticalUpside=Math.max(0,Math.min(1,0.25 + (t.includes('higher up')||t.includes('more advanced')||t.includes('second striker')||t.includes('set pieces')||t.includes('penalt') ? 0.35 : 0) + (t.includes('two positions')||t.includes('multiple positions') ? 0.15 : 0)));
  return {sentiment,roleSecurity,minutesRisk,tacticalUpside,quoteSignals:[...pos.map(x:`positive:${x}`),...neg.map(x=>`negative:${x}`)]};
}

async function search(query: string) {
  const results = await Promise.allSettled([serpSearch(query), taloredataSearch(query), webSearch(query)]);
  return results.filter(r=>r.status==='fulfilled' && r.value.success).map(r=>(r as PromiseFulfilledResult<any>).value.data).join('\n--- INDEPENDENT SOURCE ---\n').slice(0,9000);
}

export async function analyzeFplManagerIntel(players: Array<{player:string;club:string}>): Promise<{success:boolean;data:ManagerIntel[];source:string;error?:string}> {
  ...
  for(const item of players.slice(0,20)) {
    try {
      ...
      const confidence=Math.max(0.2,Math.min(0.95,(evidence.length>1200?0.82:0.55) - (freshnessDays!==null&&freshnessDays>7?0.2:0)));
      out.push({...item,...c,latestEvidence:evidence.split('\n--- INDEPENDENT SOURCE ---\n').filter(Boolean).slice(0,3),freshnessDays,confidence});
    } catch {
      out.push({player:item.player,club:item.club,sentiment:'uncertain',roleSecurity:0.5,minutesRisk:0.3,tacticalUpside:0.25,quoteSignals:[],latestEvidence:[],freshnessDays:null,confidence:0.2});
    }
  }
