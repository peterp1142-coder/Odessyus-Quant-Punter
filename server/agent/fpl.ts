...
interface EnrichedPlayer { p:FplPlayer; team:FplTeam; score:number; projected6:number; minutesRate:number; fixtureAvg:number; fixtureCount:number; managerIntel?:ManagerSignal; }

async function fetchJson<T>(url:string):Promise<T>{
  const started=Date.now();
  console.log(`[FPL HTTP] GET ${url}`);
  const controller=new AbortController(); const timer=setTimeout(()=>controller.abort(),HTTP_TIMEOUT);
  try {
    const r=await fetch(url,{headers:{Accept:'application/json','User-Agent':'Odessyus-FPL/2.1'},signal:controller.signal});
    console.log(`[FPL HTTP] ${r.status} ${url} (${Date.now()-started}ms)`);
    if(!r.ok) throw new Error(`HTTP ${r.status} for ${url}`);
    const json=await r.json() as T;
    console.log(`[FPL HTTP] JSON received from ${url} (${Date.now()-started}ms)`);
    return json;
  } catch(e) {
    const msg=e instanceof Error?e.message:String(e);
    console.error(`[FPL HTTP] FAILED ${url} after ${Date.now()-started}ms: ${msg}`);
    throw e;
  } finally { clearTimeout(timer); }
}
function numeric(v:string|number|undefined){const n=Number(v);return Number.isFinite(n)?n:0;}
function availability(p:FplPlayer){ if(p.status==='u'||p.status==='s') return 0; if(typeof p.chance_of_playing_next_round==='number') return Math.max(0,Math.min(1,p.chance_of_playing_next_round/100)); return p.status==='a'?1:0.65; }
...
export async function buildFplWeeklyTeam(input:Record<string,unknown>={}):Promise<{success:boolean;data:string;error?:string;source?:string}>{
  ...
  try{
    console.log(`[FPL] HTTP phase starting${jobId?` job=${jobId}`:''}`);
    const [data,rawFixtures]=await Promise.all([fetchJson<FplResponse>(FPL_BOOTSTRAP),fetchJson<FplFixtureRaw[]>(FPL_FIXTURES)]);
    console.log(`[FPL] Official data loaded: players=${data.elements.length} teams=${data.teams.length} events=${data.events.length} fixtures=${rawFixtures.length}${jobId?` job=${jobId}`:''}`);
    const fixtures=normalizeFixtures(rawFixtures); const gameweek=nextGameweek(data); const gameweekId=requireGameweekId(gameweek); console.log(`[FPL] Target Gameweek=${gameweekId} deadline=${gameweek?.deadline_time||'unknown'}${jobId?` job=${jobId}`:''}`);
    const teamMap=new Map(data.teams.map(t=>[t.id,t])); (globalThis as any).__fplTeamMap=teamMap; const horizon=buildHorizonFixtures(fixtures,gameweekId); console.log(`[FPL] Six-GW horizon fixtures=${horizon.length}${jobId?` job=${jobId}`:''}`);
    const viable=data.elements.filter(p=>availability(p)>0.45&&p.now_cost>0); const preliminary=viable.map(p=>({p,prof:playerFixtureProfile(p,horizon,gameweekId)})).sort((a,b)=>numeric(b.p.ep_next)-numeric(a.p.ep_next)||numeric(b.p.points_per_game)-numeric(a.p.points_per_game)); const intelCandidates=preliminary.slice(0,120).map(x=>({player:x.p.web_name,club:teamMap.get(x.p.team)?.name||'Unknown'}));
    console.log(`[FPL SEARCH] Starting manager/player research for ${intelCandidates.length} candidates`);
    const managerIntelResult=await analyzeFplManagerIntel(intelCandidates);
    console.log(`[FPL SEARCH] Manager/player research complete: ${managerIntelResult.data.length} players`);
    const intelMap=new Map<string,ManagerSignal>(); for(const i of managerIntelResult.data){const clubId=data.teams.find(t=>t.name===i.club)?.id; if(clubId!==undefined)intelMap.set(`${clubId}:${i.player}`,{roleSecurity:i.roleSecurity,minutesRisk:i.minutesRisk,tacticalUpside:i.tacticalUpside,confidence:i.confidence,sentiment:i.sentiment,freshnessDays:i.freshnessDays,quoteSignals:i.quoteSignals||[],latestEvidence:i.latestEvidence||[]}); }
    const enriched=viable.map(p=>scorePlayer(p,horizon,gameweekId,intelMap.get(`${p.team}:${p.web_name}`))).filter(x=>x.minutesRate>0.05).sort((a,b)=>b.score-a.score); console.log(`[FPL] Scored ${enriched.length} players; optimizing legal 15-man squad`);
    const opt=optimizeSquad(enriched); const plan=chooseXI(opt.selected); console.log(`[FPL] Optimized squad complete: score=${opt.score.toFixed(3)} budgetRemaining=${(opt.budgetRemaining/10).toFixed(1)} formation=${plan.formation}`);
    const currentSquad=typeof input.current_squad==='string'?String(input.current_squad).slice(0,2000):''; const fmt=(x:EnrichedPlayer)=>`${x.p.web_name} (${positionName(x.p.element_type)}, ${x.team.short_name||x.team.name}) £${(x.p.now_cost/10).toFixed(1)} | GW score ${x.score.toFixed(2)} | 6GW ${x.projected6.toFixed(2)} | FDR ${x.fixtureAvg.toFixed(2)} | fixtures ${x.fixtureCount}${x.managerIntel?` | role ${x.managerIntel.roleSecurity.toFixed(2)} | mins risk ${x.managerIntel.minutesRisk.toFixed(2)}`:''}`; const managerSignals=managerIntelResult.data.filter(i=>i.sentiment!=='neutral'||i.roleSecurity<0.65||i.minutesRisk>0.25||i.tacticalUpside>0.35).sort((a,b)=>b.confidence-a.confidence).slice(0,20); const transferGuidance=currentSquad?`Current squad supplied for transfer planning: ${currentSquad}. Compare this squad against the optimized six-Gameweek pool and prioritize high-impact upgrades; do not rebuild blindly if the current squad already satisfies the budget/club/position constraints.`:'No current squad supplied: this is a fresh-squad optimization.'; const output={mode:'FPL_DEDICATED_OPTIMIZER',gameweek:gameweekId,deadline:gameweek?.deadline_time??'unknown',budget_remaining:`£${(opt.budgetRemaining/10).toFixed(1)}m`,optimization_score:Number(opt.score.toFixed(3)),horizon_gameweeks:HORIZON,squad:opt.selected.map(fmt),starting_xi:plan.xi.map(fmt),formation:plan.formation,bench:plan.bench.map(fmt),captain:fmt(plan.captain),vice_captain:fmt(plan.vice),transfer_guidance:transferGuidance,top_manager_role_signals:managerSignals.map(i=>({player:i.player,club:i.club,sentiment:i.sentiment,role_security:i.roleSecurity,minutes_risk:i.minutesRisk,tactical_upside:i.tacticalUpside,freshness_days:i.freshnessDays,quote_signals:i.quoteSignals,confidence:i.confidence,evidence:i.latestEvidence||[]})),search_strategy:'FPL selection uses official FPL structured data first. Search is reserved for manager/player role, tactical, fitness and breaking-news context. Match referee research is supporting evidence only and never a primary squad-selection driver.'};
    return{success:true,data:JSON.stringify(output),source:'fpl_weekly_team'};
  }catch(e){ const msg=e instanceof Error?e.message:String(e); console.error(`[FPL] Optimizer failed${jobId?` job=${jobId}`:''}: ${msg}`); return{success:false,data:'',error:msg,source:'fpl_weekly_team'}; }
}