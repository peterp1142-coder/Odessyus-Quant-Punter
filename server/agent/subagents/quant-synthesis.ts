...
const PLACEHOLDER_SELECTIONS = new Set(['model lead','model target','n/a','none','unknown','no bet','no_bet','unavailable','not available']);
function isPlaceholder(value: string): boolean { return PLACEHOLDER_SELECTIONS.has(value.trim().toLowerCase()); }
function isConcreteSelection(market: string, selection: string): boolean {
  const m = market.trim().toLowerCase();
  const s = selection.trim();
  if (!m || !s || isPlaceholder(s)) return false;
  if (m.includes('over/under') || m.includes('total') || m.includes('goals')) return /^(over|under)\s+\d+(?:\.\d+)?$|^\d+(?:\.\d+)?\s+goals$/i.test(s);
...
function parseSelectionPayload(answer: string, fallback: SelectedBet) {
  ...
  if (pm) {
    for (const candidate of balancedJsonCandidates(pm[1])) {
      try {
        ...
        primary = {
          status: String(p.status || 'NO_BET'), market: p.market, selection: p.selection,
      ...
      try {
        ...
        alternatives = p.filter((x: any) => x && typeof x.market === 'string' && typeof x.selection === 'string' && !isPlaceholder(x.selection))
          ...
          .map((x: any): SelectedBet => ({
            status: String(x.status || 'CONSIDER'), market: x.market, selection: x.selection,
  ...
  if (primary.status === 'BET' && (!primary.odds || primary.odds <= 1 || !isConcreteSelection(primary.market, primary.selection))) {
    primary = noBetFallback(primary.confidence_pct, 'The synthesis returned BET without a concrete selection and verified positive betting price.', primary.probability_pct, primary.odds);
  }
