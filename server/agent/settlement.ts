...
async function fetchScore(fixture: string): Promise<ScoreResult> {
  const result = await allSportsLivescore();
  if (!result.success || !result.data) return { homeScore: -1, awayScore: -1, status: '', found: false };
...
function settleMarket(market: string, home: number, away: number): SettlementResult {
  ...
  if (m.includes('btts') || m.includes('both teams')) {
    ...
    if (m.includes('yes') && bttsYes) return { outcome: 'won', actualOutcome: 'btts_yes', roi: 0 };
    if (m.includes('no') && bttsNo) return { outcome: 'won', actualOutcome: 'btts_no', roi: 0 };
  ...
  if (m.includes('over 2.5') || m.includes('over2.5') || (m.includes('over') && m.includes('2.5'))) {
    if (totalGoals > 2.5) return { outcome: 'won', actualOutcome: 'over_2.5', roi: 0 };
  ...
  if (m.includes('under 2.5') || m.includes('under2.5') || (m.includes('under') && m.includes('2.5'))) {
    if (totalGoals < 2.5) return { outcome: 'won', actualOutcome: 'under_2.5', roi: 0 };
    return { outcome: 'lost', actualOutcome: 'over_2.5', roi: -1 };
  }

  if (m.includes('over 3.5') || m.includes('over3.5')) {
    if (totalGoals > 3.5) return { outcome: 'won', actualOutcome: 'over_3.5', roi: 0 };
    return { outcome: 'lost', actualOutcome: 'under_3.5', roi: -1 };
  }

  if (m.includes('under 3.5') || m.includes('under3.5')) {
    if (totalGoals < 3.5) return { outcome: 'won', actualOutcome: 'under_3.5', roi: 0 };
  ...
  if (m.includes('double chance') || m.includes('1x') || m.includes('x2') || m.includes('12')) {
    if (m.includes('1x') || (m.includes('home') && m.includes('draw'))) {
      if (homeWin || draw) return { outcome: 'won', actualOutcome: '1x', roi: 0 };
      return { outcome: 'lost', actualOutcome: 'away_win', roi: -1 };
    }
    if (m.includes('x2') || (m.includes('away') && m.includes('draw'))) {
      if (!homeWin) return { outcome: 'won', actualOutcome: 'x2', roi: 0 };
      return { outcome: 'lost', actualOutcome: 'home_win', roi: -1 };
    }
    if (m.includes('12') || m.includes('home or away')) {
      if (!draw) return { outcome: 'won', actualOutcome: '12', roi: 0 };
...
function settleAsianHandicap(market: string, home: number, away: number): SettlementResult {
  const m = market.toLowerCase();
  const ahMatch = m.match(/([+-]?\d+\.?\d*)/);
  if (!ahMatch) return { outcome: 'manual_review', actualOutcome: 'unknown_ah', voidReason: 'Could not parse AH line', roi: 0 };

  const line = parseFloat(ahMatch[1]);
  const isHome = !m.includes('away');
  const margin = isHome ? home - away + line : away - home + line;

  if (margin > 0.25) return { outcome: 'won', actualOutcome: `ah_${isHome ? 'home' : 'away'}_${line}`, roi: 0 };
  if (margin < -0.25) return { outcome: 'lost', actualOutcome: `ah_${isHome ? 'home' : 'away'}_${line}`, roi: -1 };
  if (margin === 0) return { outcome: 'push', actualOutcome: `ah_${isHome ? 'home' : 'away'}_${line}`, voidReason: 'AH push — stake refunded', roi: 0 };

  if (margin === 0.25) return { outcome: 'half_loss', actualOutcome: `ah_${isHome ? 'home' : 'away'}_${line}`, voidReason: 'AH half-loss', roi: -0.5 };
  if (margin === -0.25) return { outcome: 'half_win', actualOutcome: `ah_${isHome ? 'home' : 'away'}_${line}`, voidReason: 'AH half-win', roi: 0.5 };

  return { outcome: 'manual_review', actualOutcome: 'unknown_ah', voidReason: `Unresolved AH margin: ${margin}`, roi: 0 };
}

async function applySettlement(pick: PendingPick, settlement: SettlementResult, score: ScoreResult): Promise<void> {
