...
function getStatusFromStep(step) {
    ...
    for (const [prefix, label] of Object.entries(SUBAGENT_STATUS)) {
        if (step.content.includes(prefix))
            return label;
    }
    return null;
}
function formatForTelegram(finalAnswer, meta) {
...
export function initTelegram() {
    ...
    bot.start((ctx) => ctx.reply('Welcome to Odessyus v2.0 — Multi-Subagent Sports Intelligence\n\n' +
    ...
    bot.help((ctx) => ctx.reply('Odessyus v2.0 — 80-Feature Multi-Subagent Engine\n\n' +
    ...
    bot.command('stats', async (ctx) => {
      ...
      `);
            const s = rows[0] || {};
            await ctx.reply(`Daily Efficiency Report\n\n` +
            ...
            await ctx.reply('Unable to load stats. Please try again.');
        }
    });
    bot.on('text', async (ctx) => {
        ...
        try {
            await query('INSERT INTO conversations (id, session_id, channel, role, content) VALUES (?, ?, ?, ?, ?)', [uuidv4(), sessionId, 'telegram', 'user', text]).catch(() => { });
            const result = await runOrchestrator(text, sessionId, async (step) => {
            ...
            if (result.success && result.finalAnswer) {
                ...
                await query(`INSERT INTO predictions
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?, ?, ?)`, [
                ...
                try {
                    ...
                    await query(`INSERT INTO feature_vectors
              (id, prediction_id,
             ...
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
                    ...
                    console.error('[Telegram] Save feature_vectors:', fvErr);
                }
                await query('INSERT INTO conversations (id, session_id, channel, role, content) VALUES (?, ?, ?, ?, ?)', [uuidv4(), sessionId, 'telegram', 'assistant', result.finalAnswer]).catch(() => { });
            }
            else {
            ...
            await ctx.reply(isRate
                ? 'AI model is rate-limited. Please wait ~1 minute and try again.'
                : 'Temporary error. Please try again in a moment.').catch(() => { });
        }
    });
