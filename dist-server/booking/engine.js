 ...
 */
async function discoverSelectorBooking(page, kind, hint) {
    const result = await page.evaluate((k, h) => {
        const scored = [];
        function makeSelector(el) {
            ...
            if (cls && typeof cls === 'string') {
                ...
                if (firstClass)
                    return `${el.tagName.toLowerCase()}.${firstClass.replace(/[^a-zA-Z0-9_-]/g, '')}`;
            }
            return el.tagName.toLowerCase();
        }
        const elements = Array.from(document.querySelectorAll('input, button, a, div, span, [role="button"], [role="textbox"], [contenteditable]'));
            ...
            switch (k) {
                case 'username':
                    if (type === 'email' || type === 'tel' || type === 'text')
                        score += 3;
                    if (placeholder.includes('email') || placeholder.includes('phone') || placeholder.includes('user') || placeholder.includes('account'))
                        score += 4;
                    if (name.includes('email') || name.includes('phone') || name.includes('user') || name.includes('login'))
                        score += 4;
                    if (cls.includes('user') || cls.includes('email') || cls.includes('phone') || cls.includes('login'))
                        score += 2;
                    if (el.tagName === 'INPUT')
                        score += 1;
                    break;
                ...
                case 'loginButton':
                    ...
                    if (type === 'submit')
                        score += 5;
                    if (text.match(/log\s*in|sign\s*in|enter|submit|continue/i))
                        score += 5;
                    if (cls.includes('login') || cls.includes('submit') || cls.includes('btn'))
                        score += 2;
                    break;
                case 'searchBox':
                    if (el.tagName === 'INPUT')
                    ...
                    if (type === 'search' || type === 'text')
                        score += 2;
                    if (placeholder.includes('search') || placeholder.includes('find') || placeholder.includes('team') || placeholder.includes('match'))
                        score += 4;
                ...
                case 'stakeInput':
                    ...
                    if (type === 'number' || type === 'text')
                        score += 2;
                    if (placeholder.includes('stake') || placeholder.includes('amount') || placeholder.includes('bet'))
                        score += 5;
                    if (name.includes('stake') || name.includes('amount'))
                        score += 3;
                    if (cls.includes('stake') || cls.includes('amount'))
                        score += 2;
                    if (ariaLabel.includes('stake') || ariaLabel.includes('amount'))
                        score += 3;
                    break;
                case 'confirmButton':
                    if (el.tagName === 'BUTTON' || el.getAttribute('role') === 'button')
                    ...
                    if (type === 'submit')
                        score += 3;
                    if (text.match(/place\s*bet|confirm|submit|bet\s*now|accept/i))
                        score += 6;
                    if (cls.includes('place') || cls.includes('confirm') || cls.includes('submit') || cls.includes('bet'))
                        score += 3;
                    break;
                case 'confirmationMsg':
                    if (text.match(/success|confirmed|accepted|placed|bet\s*id|booking|ticket|receipt|reference/i))
                        score += 5;
                    if (cls.includes('success') || cls.includes('confirm') || cls.includes('receipt') || cls.includes('message'))
                        score += 3;
                    if (text.length > 10 && text.length < 500)
                        score += 1;
                    break;
                case 'cookieBanner':
                    if (text.match(/cookie|consent|accept\s*all|agree/i))
                        score += 5;
 ...
 */
async function waitForSelectorWithFallback(page, primarySelector, kind, timeout = 10000, hint) {
...
async function dismissCookies(page, platform) {
...
async function login(page, platform, username, password) {
    try {
...
async function findAndBookSportyBet(page, req, platform) {
    ...
    const selectionEl = await page.evaluateHandle((selectionText, market) => {
        const allEls = Array.from(document.querySelectorAll('*'));
        return allEls.find(el => el.textContent?.includes(selectionText) &&
            (el.classList.contains('odd') || el.classList.contains('odds') ||
                el.getAttribute('data-odd') || el.closest('[class*="odds"]'))) || null;
    ...
    if (selectionEl) {
        ...
        await selectionEl.click();
        await delay(2000);
    }
    return { oddsObtained, betId: '', confirmText: 'Selection added to betslip' };
}
async function enterStakeAndConfirm(page, platform, stakeAmount) {
    // Wait for betslip stake input — with scrape-then-select fallback
    const stakeSel = await waitForSelectorWithFallback(page, platform.selectors.betslipStakeInput, 'stakeInput', 10000, 'stake');
    await delay(1000);
    ...
    const confirmSel = await waitForSelectorWithFallback(page, platform.selectors.betslipConfirmButton, 'confirmButton', 8000, 'place bet');
    ...
    try {
        if (platform.selectors.confirmationMsg) {
            try {
                const confirmMsgSel = await waitForSelectorWithFallback(page, platform.selectors.confirmationMsg, 'confirmationMsg', 8000, 'success');
...
function createGenericPlatform(id) {
...
export async function placeBet(req) {
    ...
    try {
        ...
        try {
            if (req.platform === 'sportybet') {
                ...
                if (platform.selectors.searchBox) {
                    try {
                        const searchSel = await waitForSelectorWithFallback(page, platform.selectors.searchBox, 'searchBox', 8000, 'search');
                        await page.type(searchSel, req.fixture, { delay: 50 });
                        await delay(2000);
                    }
                    catch { /* no search box */ }
