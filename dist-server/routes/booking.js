...
router.post('/place', async (req, res) => {
    const body = req.body;
    const platform = (body.platform || process.env.BOOKING_PLATFORM || 'sportybet');
    ...
    try {
        ...
        // Save to DB
        try {
            await query(`INSERT INTO booking_log
...
router.get('/history', async (_req, res) => {
    try {
        const rows = await query(`SELECT id, prediction_id, platform, fixture, market, selection,
              odds_obtained, stake_amount, potential_return, bet_id,
