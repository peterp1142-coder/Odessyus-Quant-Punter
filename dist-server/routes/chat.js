...
router.post('/', async (req, res) => {
    const { message, sessionId: existing } = req.body;
    if (!message?.trim())
    ...
    try {
        ...
        console.error('[Chat] Save user msg:', err);
    }
...
router.get('/stream/:sessionId', async (req, res) => {
    ...
    const cleanup = () => { clearInterval(heartbeat); if (!res.writableEnded)
        res.end(); };
    req.on('close', () => clearInterval(heartbeat));
    sse('connected', { sessionId, engine: 'orchestrator-v2' });
    let finalAnswer = '';
    try {
        const result = await runOrchestrator(message.trim(), sessionId, (step) => sse('step', step));
        ...
        if (result.success && result.finalAnswer) {
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?, ?, ?)`, [
             ...
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
                ...
                console.error('[Chat] Save prediction:', e);
            }
        }
    }
    catch (err) {
...
router.get('/history/:sessionId', async (req, res) => {
    try {
        const rows = await query('SELECT id, role, content, created_at FROM conversations WHERE session_id = ? ORDER BY created_at ASC LIMIT 100', [req.params.sessionId]);
