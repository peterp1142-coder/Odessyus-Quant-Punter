...
function parseReActResponse(content) {
    ...
    if (!rawAction) {
        if (trimmed.length > 300)
            return { type: 'final', thought, finalContent: trimmed };
    ...
    try {
        ...
        if (!Object.keys(toolInput).length) {
            ...
            if (bare.startsWith('http'))
                ...
                toolInput.query = bare;
        }
    }
    return { type: 'action', thought, toolName: rawAction, toolInput };
}
export async function runReActLoop(userQuery, sessionId, onStep, conversationHistory = []) {
    const steps = [];
    const emit = (step) => {
    ...
    try {
        ...
        while (iteration < MAX_ITERATIONS) {
            ...
            // Action step
            if (parsed.thought)
                emit({ type: 'thought', content: parsed.thought, iteration });
            if (!parsed.toolName) {
        ...
        const finalResp = await mistralPool.call(client => client.chat.complete({
            model: 'mistral-large-latest',
            messages: messages,
            temperature: 0.1,
            maxTokens: 5000,
...
export function extractPredictionData(finalAnswer) {
...
//# sourceMappingURL=react-engine.js.map