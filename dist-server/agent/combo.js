...
export function buildCombo(legs) {
...
function computeCorrelationDiscount(legs) {
    let maxDiscount = 0;
    for (let i = 0; i < legs.length; i++) {
        for (let j = i + 1; j < legs.length; j++) {
            for (const pair of CORRELATION_PAIRS) {
                ...
                if ((pair.a.test(mi) && pair.b.test(mj)) ||
...
export function selectBestCombo(allLegs) {
    if (allLegs.length < 2)
        return null;
