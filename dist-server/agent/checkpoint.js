...
function filePath(sessionId, agentName) {
...
export async function loadCheckpoint(sessionId, agentName) {
    try {
        const raw = await fs.readFile(filePath(sessionId, agentName), 'utf8');
        const cp = JSON.parse(raw);
        if (cp.version !== 2 && cp.version !== 3)
            return null;
...
export async function clearCheckpoint(sessionId, agentName) {
    try {
        await fs.unlink(filePath(sessionId, agentName));
    }
    catch { /* doesn't exist */ }
}
    ...
    try {
        ...
        for (const file of files) {
            ...
            try {
                ...
                const cp = JSON.parse(raw);
                if (!cp.savedAt || Date.now() - cp.savedAt > TTL_MS)
                    await fs.unlink(fp);
            }
            catch {
