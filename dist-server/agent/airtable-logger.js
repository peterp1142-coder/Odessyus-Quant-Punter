...
/* -------------------- UTIL -------------------- */
const safeNumber = (n) => typeof n === 'number' && !isNaN(n) ? n : undefined;
const toISO = (d) => {
    if (!d)
        return undefined;
...
async function register(baseId, name, id) {
    await query(`INSERT INTO airtable_registry (base_id, table_name, table_id)
     VALUES (?, ?, ?)
     ON DUPLICATE KEY UPDATE table_id = VALUES(table_id)`, [baseId, name, id]);
}
...
export async function initAirtable() {
    ...
    if (!baseId) {
        const rows = await query('SELECT base_id FROM airtable_registry LIMIT 1');
        if (rows.length)
    ...
    for (const t of schema.tables) {
        tables[t.name] = t.id;
        await register(baseId, t.name, t.id);
    }
    return { baseId, tables };
}
/* -------------------- LOGGING -------------------- */
export async function logPrediction(data) {
    const init = cache || (cache = await initAirtable());
    if (!init)
        return;
    const tableId = init.tables['Predictions'];
    if (!tableId) {
        console.error('Missing Predictions table ID');
        return;
    }
    const fields = cleanFields({
        ...
        'Predicted Prob %': safeNumber(data.predictedProb * 100),
        'Implied Prob %': safeNumber(data.impliedProb * 100),
        'EV %': safeNumber(data.evPct * 100),
        'Star Rating': safeNumber(data.starRating),
        'Recommended Odds': safeNumber(data.recommendedOdds),
        'Data Completeness': safeNumber(data.dataCompleteness),
        'Is Value Bet': data.isValueBet,
        'Recommended Stake': data.recommendedStake,
    ...
    await airtableFetch(`/${init.baseId}/${tableId}`, {
        method: 'POST',
        body: JSON.stringify({ records: [{ fields }] }),
    });
}
export async function logResult(data) {
    const init = cache || (cache = await initAirtable());
    if (!init)
        return;
    const tableId = init.tables['Results'];
    if (!tableId) {
        console.error('Missing Results table ID');
        return;
    }
    const fields = cleanFields({
        'Prediction ID': data.predictionId,
        'Fixture': data.fixture,
        'Market': data.market,
        'Actual Outcome': data.actualOutcome,
        'Result': data.result,
...
//# sourceMappingURL=airtable-logger.js.map