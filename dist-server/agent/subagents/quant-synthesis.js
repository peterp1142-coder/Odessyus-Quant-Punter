...
async function synthesiseMultiFixture(args) {
    ...
    try {
        ...
        if (!oddsPicksText)
            oddsPicksText = raw.substring(0, 3000);
    }
    catch {
...
Use FINAL_ANSWER: format. Be concise but complete.`;
    try {
        ...
        if (typeof content !== 'string' || !content)
            throw new Error('Empty multi-fixture synthesis response');
...
export async function runQuantSynthesis(opts) {
    ...
    if (existing?.rawOutput) {
        ...
        const cd = existing.accumulatedData;
        const monteCarlo = cd?.monteCarlo || { home: 0.4, draw: 0.28, away: 0.32, stdDev: 0.05 };
        await clearCheckpoint(sessionId, agentName);
        return {
            finalAnswer, steps, success: true, monteCarlo,
            trueProb: cd?.trueProb || 0.5,
            impliedProb: cd?.impliedProb || 0.45,
            expectedValue: cd?.expectedValue || 0,
            starRating: cd?.starRating || 1,
            dataCompletenessScore: cd?.dataCompletenessScore || 50,
            isValueBet: cd?.isValueBet ?? false,
            recommendedStake: cd?.recommendedStake || 'MONITOR',
            recommendedOdds: cd?.recommendedOdds || 2.0,
            confidence: cd?.confidence || 50,
            goalStatement: cd?.goalStatement || `${fixture} — ${market}`,
            categoryProbabilities: cd?.categoryProbabilities || { market: 0, form: 0, injury: 0, sentiment: 0 },
        };
    }
    ...
    emit({ type: 'thought', content: `Features extracted — ` +
    ...
    emit({ type: 'thought', content: `Monte Carlo (${mcResult.simCount.toLocaleString()} sims) — ` +
    ...
    emit({ type: 'thought', content: `Gate: ${isValueBet ? '✅ PASS' : `❌ FAIL — ${scored.gateFailReason}`} | ` +
...
Use FINAL_ANSWER: format. Embed every exact probability number above. Be honest — if data quality is low, say so.`;
    try {
        ...
        emit({ type: 'thought', content: `✅ Final: True ${(trueProb * 100).toFixed(1)}% | Implied ${(impliedProb * 100).toFixed(1)}% | ` +
...
//# sourceMappingURL=quant-synthesis.js.map