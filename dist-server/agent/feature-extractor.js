...
Extract and return this exact JSON (numbers only, no strings for numeric fields):
{
  ...
  "sentimentDataQuality": <0-1>
    ...
    try {
        ...
        const formDataQuality = clamp((formFieldsPresent.filter(Boolean).length / formFieldsPresent.length) * 0.85 + formTextBonus, 0, 1);
        ...
        const oddsDataQuality = clamp((marketFieldsPresent.filter(Boolean).length / marketFieldsPresent.length) * 0.85 + oddsTextBonus, 0, 1);
        ...
        const injuryDataQuality = clamp((injuryFieldsPresent.filter(Boolean).length / injuryFieldsPresent.length) * 0.8 + injuryTextBonus, 0, 1);
        ...
        const sentimentDataQuality = clamp((sentimentFieldsPresent.filter(Boolean).length / sentimentFieldsPresent.length) * 0.8 + sentimentTextBonus, 0, 1);
        const form = {
