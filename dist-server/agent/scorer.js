...
function scoreForm(s, mcProb, market) {
    let p = mcProb;
    ...
    if (m.includes('btts') || m.includes('both teams')) {
        ...
        if (m.includes('over'))
            p = 0.50 + (goalSum - 2.5) * 0.12;
        else
            p = 0.50 - (goalSum - 2.5) * 0.12;
    }
    return Math.max(0.05, Math.min(0.95, p));
}
function scoreInjury(s, baseMCProb) {
...
function scoreSentiment(s, baseMCProb) {
...
// ─── Overround removal ────────────────────────────────────────────────────────
export function removeOverround(decimalOdds, estimatedMargin = 0.05) {
    if (decimalOdds <= 1)
        return 0.5;
...
// ─── Kelly Criterion ──────────────────────────────────────────────────────────
export function calcKelly(trueProbability, decimalOdds) {
...
// ─── Data completeness ────────────────────────────────────────────────────────
function computeCompleteness(ms, fs, is_, ss) {
    const breakdown = {
    ...
    const score = breakdown.market * WEIGHTS.market +
...
function computeStarRating(evPct, completeness) {
    // Penalise low data completeness
    const penalisedEV = evPct * Math.min(1, completeness / 70);
    if (penalisedEV >= 15)
        return { stars: 5, stake: 'MAX' };
    if (penalisedEV >= 10)
        return { stars: 4, stake: 'CONFIDENT' };
    if (penalisedEV >= 5)
        return { stars: 3, stake: 'STANDARD' };
    if (penalisedEV >= 2)
        return { stars: 2, stake: 'SMALL' };
    if (penalisedEV >= 0)
        return { stars: 1, stake: 'MONITOR' };
    return { stars: 1, stake: 'SKIP' };
}
// ─── Main scorer ──────────────────────────────────────────────────────────────
export function score(input) {
    const msKeys = ['impliedProbHome', 'reverseLM', 'lineMovement', 'oddsDataQuality', 'openingOddsHome', 'currentOddsHome', 'pinnacleOdds', 'overround'];
    const fsKeys = ['xgHome', 'xgAway', 'xgConcededHome', 'xgConcededAway', 'formPtsHome', 'formPtsAway', 'h2hWinRateHome', 'formDataQuality', 'homeWinPctVenue', 'awayWinPctVenue', 'streakHome', 'streakAway', 'restDaysHome', 'restDaysAway'];
    const isKeys = ['injuryIndexHome', 'injuryIndexAway', 'gtdRiskHome', 'gtdRiskAway', 'injuryDataQuality', 'absentPlayerRatingHome', 'absentPlayerRatingAway'];
    const ssKeys = ['motivationHome', 'motivationAway', 'weatherImpact', 'refBias', 'sentimentDataQuality', 'crowdFactor'];
    const ms = fill(input.marketSignals, msKeys);
    const fs = fill(input.formSignals, fsKeys);
    const is_ = fill(input.injurySignals, isKeys);
    const ss = fill(input.sentimentSignals, ssKeys);
    // Derive base probability from MC for the target market
    const market = input.market.toLowerCase();
    let mcProb;
    if (market.includes('draw'))
        mcProb = input.mcResult.draw;
    else if (market.includes('away'))
        mcProb = input.mcResult.awayWin;
    else if (market.includes('btts') || market.includes('both teams'))
        mcProb = input.mcResult.btts;
    else if (market.includes('over 3.5') || market.includes('over3.5'))
        mcProb = input.mcResult.over35;
    else if (market.includes('under 3.5') || market.includes('under3.5'))
        mcProb = input.mcResult.under35;
    else if (market.includes('over 2.5') || market.includes('over2.5') || market.includes('over'))
        mcProb = input.mcResult.over25;
    else if (market.includes('under 2.5') || market.includes('under2.5') || market.includes('under'))
        mcProb = input.mcResult.under25;
    else if (market.includes('dnb') && market.includes('home'))
        mcProb = input.mcResult.dnbHome;
    else if (market.includes('dnb') && market.includes('away'))
        mcProb = input.mcResult.dnbAway;
    else
        mcProb = input.mcResult.homeWin;
    const catScores = {
    ...
    const finalProbability = Math.max(0.05, Math.min(0.95, catScores.market * WEIGHTS.market +
        catScores.form * WEIGHTS.form +
        catScores.injury * WEIGHTS.injury +
        catScores.sentiment * WEIGHTS.sentiment));
    const { score: dataCompletenessScore, breakdown: completenessBreakdown } = computeCompleteness(ms, fs, is_, ss);
    ...
    // Star rating
    const { stars: starRating, stake: recommendedStake } = computeStarRating(expectedValuePct, dataCompletenessScore);
    // Accuracy gate
    let isValueBet = true;
    let gateFailReason;
