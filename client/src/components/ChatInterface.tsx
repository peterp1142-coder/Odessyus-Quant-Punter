...
const QUICK_PROMPTS=[{icon:'⚽',text:'Best value bets in Premier League today'},{icon:'🏆',text:"Analyze Champions League tonight's fixtures"},{icon:'📊',text:'Top 3 over/under picks this weekend'},{icon:'🎯',text:'Give me BTTS tips for today'},{icon:'👑',text:'Build my FPL team for this Gameweek'}];
const AGENTS=[
 {icon:'📊',name:'OddsScout',desc:'Exact-market prices & line movement'},
 {icon:'📈',name:'FormScout',desc:'xG, venue form & useful H2H'},
 {icon:'🏥',name:'InjuryIntel',desc:'Availability & suspension intelligence'},
 {icon:'📰',name:'SentimentAgent',desc:'Team news, motivation & context'},
 {icon:'🧑‍⚖️',name:'RefereeScout',desc:'Assigned-referee specific evidence'},
 {icon:'💹',name:'MarketMicrostructureScout',desc:'Liquidity, dispersion & price integrity'},
 {icon:'🪖',name:'LineupScout',desc:'Confirmed / predicted XI'},
 {icon:'🧠',name:'TacticalScout',desc:'Matchup, transitions & set pieces'},
 {icon:'🛡️',name:'ModelRiskScout',desc:'Adversarial stress testing'},
 {icon:'🧪',name:'DataQualityScout',desc:'Freshness, provenance & contradictions'},
...
export const ChatInterface:React.FC<ChatInterfaceProps>=({conversationId,initialMessages,onMessagesChange,onNewChat})=>{
 ...
 const isEmpty=messages.length===0;const verificationRequired=verification.status==='required';const verificationResuming=verification.status==='resuming';
 const lastMeta:any=messages.length?(messages[messages.length-1] as any).metadata:undefined;
 const agentPlan=Array.isArray(lastMeta?.agentPlan)?lastMeta.agentPlan:AGENTS.map(a=>({name:a.name}));
 const queueSnapshot=(lastMeta?.queueSnapshot||{}) as Record<string,{queued:number;active:number;concurrency:number}>;
 ...
 return <div className="flex flex-col h-full bg-[#0d0d0d]">
  ...
  <div className="flex-1 overflow-y-auto scrollable">
   ...
   {liveBrowserVisual&&isStreaming&&!verificationRequired&&(<div className="max-w-3xl mx-auto px-4 pt-4"><div className="rounded-2xl overflow-hidden border border-amber-500/20 bg-black/40"><div className="flex items-center justify-between gap-3 px-3 py-2 border-b border-white/[0.06] bg-white/[0.03]"><div className="flex items-center gap-2 min-w-0"><span className="w-2 h-2 rounded-full bg-amber-400"/><span className="text-[11px] font-mono text-amber-300">BROWSER PAUSED</span><span className="text-[10px] text-neutral-600 truncate">Search-only mode; browser/scraper is disabled.</span></div></div></div></div>)}
   {isEmpty?<div className="flex flex-col items-center justify-center h-full px-6 py-10 max-w-3xl mx-auto"><div className="mb-7 text-center"><div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center text-3xl mx-auto mb-5 shadow-2xl shadow-emerald-900/40">⚡</div><h1 className="text-2xl font-bold text-white mb-2">How can I help you?</h1><p className="text-neutral-500 text-sm max-w-sm mx-auto">Autonomous, fixture-scoped quantitative sports analysis using search APIs, evidence gates and calibration.</p></div><button type="button" onClick={runAccumulator} disabled={isStreaming} className="w-full mb-5 group text-left rounded-2xl border border-emerald-500/20 bg-gradient-to-br from-emerald-500/[0.10] to-teal-500/[0.05] hover:border-emerald-400/35 disabled:opacity-50 transition-all p-4"><div className="flex items-center gap-3"><div className="w-11 h-11 rounded-xl bg-emerald-500/15 border border-emerald-400/20 flex items-center justify-center text-xl">📐</div><div className="flex-1 min-w-0"><div className="text-sm font-semibold text-emerald-200">Statistically Plausible Accumulators for Today</div><div className="text-xs text-neutral-500 mt-1">Wide fixture discovery · exact-market odds · calibration · model-risk · portfolio correlation.</div></div><div className="text-emerald-400 text-lg">→</div></div></button><div className="agent-grid">{AGENTS.map(a=><div key={a.name} className="flex items-start gap-2.5 bg-white/[0.03] border border-white/[0.06] rounded-xl p-3"><span className="text-lg flex-shrink-0 mt-0.5">{a.icon}</span><div><div className="text-xs font-semibold text-neutral-200">{a.name}</div><div className="text-[11px] text-neutral-600 mt-0.5 leading-tight">{a.desc}</div></div></div>)}</div><div className="prompt-grid mt-4">{QUICK_PROMPTS.map(p=><button key={p.text} onClick={()=>{setInput(p.text);inputRef.current?.focus();}} className="flex items-center gap-3 px-4 py-3 rounded-xl bg-white/[0.03] border border-white/[0.06] text-left"><span className="text-base">{p.icon}</span><span className="text-xs text-neutral-400">{p.text}</span></button>)}</div></div>:<div className="max-w-3xl mx-auto py-6 px-4 space-y-1">{messages.map((msg,i)=><MessageBubble key={msg.id} message={msg} isActiveStreaming={isStreaming&&i===messages.length-1}/>) }{isStreaming&&messages[messages.length-1]?.content===''&&<div className="flex items-start gap-3 px-1 animate-fade-in-up py-4"><div className="w-8 h-8 rounded-full bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center text-xs font-bold text-white flex-shrink-0 mt-0.5">⚡</div><div className="flex gap-1 items-center mt-2"><span className="typing-dot"/><span className="typing-dot"/><span className="typing-dot"/></div></div>}<div ref={messagesEndRef}/></div>}
  </div>
  <div className="flex-shrink-0 px-4 pb-5 pt-3 border-t border-white/[0.06]"><div className="max-w-3xl mx-auto"><form onSubmit={handleSubmit} className="relative"><textarea ref={inputRef} value={input} onChange={e=>{setInput(e.target.value);e.target.style.height='auto';e.target.style.height=Math.min(e.target.scrollHeight,200)+'px';}} onKeyDown={handleKeyDown} placeholder="Ask for predictions, analysis, value bets or FPL…" rows={1} disabled={isStreaming||verificationRequired} className="w-full bg-white/[0.05] border border-white/[0.1] rounded-2xl px-5 py-3.5 pr-14 text-sm text-neutral-100 placeholder-neutral-600 resize-none outline-none leading-relaxed" style={{minHeight:'52px',maxHeight:'200px'}}/><div className="absolute bottom-2.5 right-2.5">{isStreaming?<button type="button" onClick={cancelRequest} className="w-9 h-9 rounded-xl bg-white/[0.08] text-neutral-300 flex items-center justify-center" title="Stop"><svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><rect x="6" y="6" width="12" height="12" rx="2"/></svg></button>:<button type="submit" disabled={!input.trim()} className="w-9 h-9 rounded-xl bg-emerald-600 disabled:bg-white/[0.06] text-white disabled:text-neutral-600 flex items-center justify-center" title="Send"><svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path d="M12 19V5M5 12l7-7 7 7"/></svg></button>}</div></form><p className="text-[11px] text-neutral-700 mt-2 text-center font-mono">11-specialist fixture DAG · search APIs · calibration & portfolio gates · browser paused</p></div></div>
 </div>;
};
